...
export default function ActivityList({ activities, onSelect, highlightIds }) {
  if (!activities?.length) {
    return <p className="muted">Nessuna attività caricata ancora.</p>
  }
  return (
          ...
          {activities.map((a) => {
            ...
            return (
              <tr
                ...
                title={onSelect ? "Apri il dettaglio dell'attività" : undefined}
              >
                <td>
                  {isNew && <span className="zone-badge zone-sicura" style={{ marginRight: 6 }}>nuova</span>}
                  {fmtDate(a.started_at)}
                </td>
                <td className="muted" title={activityTypeLabel(a.activity_type)}>{activityTypeIcon(a.activity_type)} {activityTypeLabel(a.activity_type)}</td>
