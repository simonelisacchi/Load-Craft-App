...
export function parseGpx(text) {
  ...
  const trkNameEl = trkEl?.getElementsByTagName('name')[0]
  const activityName = trkNameEl?.textContent?.trim() || null

  // <trk><type> è un campo libero e non sempre presente (dipende
  // dall'app che ha generato il GPX): quando c'è, è un indizio utile
  // sul tipo di attività; quando manca non tentiamo di indovinare.
  const trkTypeEl = trkEl?.getElementsByTagName('type')[0]
  const trkType = trkTypeEl?.textContent?.trim() || null
  ...
  return {
    ...
    distanceM: totalDist,
    record: smoothed,
    sportHint: trkType ? { kind: 'gpx', raw: trkType } : null,
  }
}
