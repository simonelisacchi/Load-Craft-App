...
export async function syncActivitiesToSheet(spreadsheetId, activities) {
  ...
  await ensureSheetHeader(spreadsheetId, token, 'Attivita', [
    'Data', 'Nome', 'Fonte', 'Durata (s)', 'Distanza (m)', 'FC media', 'FC max', 'Passo medio (s/km)', 'Carico (TRIMP)', 'VO2max stimato', 'Affidabilita',
  ])
  const rows = activities.map((a) => [
    a.started_at, a.name, a.source, a.duration_s, a.distance_m, a.avg_hr, a.max_hr, a.avg_pace_s_per_km, a.training_load, a.vo2max_estimate, a.vo2max_confidence,
  ])
